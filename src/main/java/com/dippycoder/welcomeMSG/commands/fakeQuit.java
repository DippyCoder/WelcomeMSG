...
public class fakeQuit implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(Main.config.getBoolean("fake-quit.enabled") == true) {
            if (sender.hasPermission(Main.config.getString("fake-quit.permission"))) {
                ...
                if (sender instanceof Player) {
                    ...
                    if (args.length == 1) {
                        String fname = args[0];
                        Bukkit.broadcastMessage(Main.config.getString("prefix") + Main.config.getString("player-quit.text-start") + fname + Main.config.getString("player-quit.text-end"));
                    } else {
